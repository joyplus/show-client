...
//		UmengUpdate.update(this);
	...
	public void requsetFouces(boolean isLeftSideFocus){
		if(isLeftSideFocus){
			...
			getView().findViewById(R.id.haier_1_3).requestFocus();
		}
	...
	public boolean onKey(View v, int keyCode, KeyEvent event) {
		...
		case R.id.layout_bangdan_haier:
			if(keyCode == KeyEvent.KEYCODE_DPAD_LEFT&&event.getAction() == KeyEvent.ACTION_DOWN){
				if(mPageController!=null){
					mPageController.showPage(PageController.PAGE_SKYWORTH, false);
					return true;
				}
				return false;
			}else{
		...
		case R.id.haier_3_3:
			if(keyCode == KeyEvent.KEYCODE_DPAD_RIGHT&&event.getAction() == KeyEvent.ACTION_DOWN){
				if(mPageController!=null){
					mPageController.showPage(PageController.PAGE_TCL, true);
					return true;
				}else{
				}
				return false;
